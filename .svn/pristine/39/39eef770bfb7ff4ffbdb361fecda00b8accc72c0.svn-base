...
{
    static int histogram[64];
    memset(histogram, 0x0, 64 * sizeof(int));
    
    static int brightPixelArray[256];
    memset(brightPixelArray, 0x0, 256 * sizeof(int));
...
//    glReadPixels(0, 0, 640, 480, GL_RGBA, GL_UNSIGNED_BYTE, rawImagePixels);

    NSUInteger luminanceTotal = 0;
    NSUInteger byteIndex = 0;
    for (NSUInteger currentPixel = 0; currentPixel < totalNumberOfPixels; currentPixel++)
    {
        int brightness = (rawImagePixels[byteIndex] + rawImagePixels[byteIndex + 1] + rawImagePixels[byteIndex + 2]) / 3;
        histogram[brightness / 4]++;
        brightPixelArray[brightness]++;
    ...
    {
        _luminosityProcessingFinishedBlock(normalizedLuminosityTotal, frameTime, histogram, darkest, brightest);
    }
}
